...
parser = argparse.ArgumentParser(description='run rhotermpred')
parser.add_argument('--input','-i',type=str,required=True, help='input genome sequence')
parser.add_argument('--output-directory','-o',type=str, required=True, help = 'directory to save RDT prediction')
args = parser.parse_args()

if not os.path.exists(args.output_directory):
    os.mkdir(args.output_directory)

file_genome = args.input
...
try:
    ...
    for sq in sequences_to_analyze:
        ...
        predictions_file = open(os.path.join(args.output_directory,f'predictions_coordinates_{sq}.csv'), mode='a')
        writer = csv.writer(predictions_file, delimiter='\t')
        writer.writerow(['Region', 'Start RUT', 'End RUT', 'Score' , 'Strand'])
        p = open(os.path.join(args.output_directory,f"info_about_predictions_{sq}.txt"), "a")
                ...
                if c_over_g > 1:
                    if re.search(pattern1, w):
                        ...
                        if ctrl > 0 or re.search(pattern_pause_site1, s):
                            ...
                            if final_score == 0:
                                final_score = score + 3
                            writer.writerow([f'T{cod}', rut_start, rut_end, final_score, 'plus'])
                            cod += 1
                ...
                if c_over_g > 1:
                    if re.search(pattern2, w):
                        ...
                        if ctrl > 0 or re.search(pattern_pause_site2, s):
                            ...
                            if final_score == 0:
                                final_score = score + 3
                            writer.writerow([f'T{cod}', rut_start, rut_end, final_score, 'minus'])
                            cod += 1
                            scores.append(final_score)
...
fout = open(os.path.join(args.output_directory,"rhoTermPred.bed"),"w")
for file_name in os.listdir(args.output_directory):
    if not file_name.startswith("predictions_coordinates_"):
        continue
    path = os.path.join(args.output_directory,file_name)
    seq_id = file_name[24:-4]
    print(path)
    with open(path) as f:
        for line in f:
            ...
            start, end, score = fields[1:4]
            strand = {"plus":"+","minus":"-"}[fields[4]]
            print(seq_id,start,end,fields[0],score,strand,sep="\t",file=fout)
fout.close()

print(os.path.join(args.output_directory,"rhoTermPred.bed"))
