...
def main():
    ...
    parser.add_argument('--percentile','-p',type=float,default=None,help="percentile to consider")
    ...
    if args.percentile is not None:
        ...
        with open(args.input) as f:
            for line in f:
                seq_id, start, end, _, scores, strand = line.strip().split("\t")[:6]
                scores = np.array(scores.split(",")).astype(float)
                score = np.mean(scores)
                all_scores.append(score) 
        all_scores = sorted(all_scores)
        cutoff = all_scores[int(len(all_scores)*args.percentile)]
    else:
        cutoff = -np.inf
    for line in fin:
        seq_id, start, end, _, scores, strand = line.strip().split("\t")[:6]
